---
title: Curriclum Vitae
permalink: /cv/
---

<!-- <object data="https://smbolinger.github.io/assets/documents/bolinger_CV.pdf" type="application/pdf" width="300" height="200"> -->
<!--   alt : <a href="https://smbolinger.github.io/assets/documents/bolinger_CV.pdf">bolinger_CV.pdf</a> -->
<!-- </object> -->

<!-- <object data="{{ site.baseurl }}/assets/documents/bolinger_CV.pdf#toolbar=0&navpanes=0" type="application/pdf" width="100%" height="80%"> -->
<!-- <object data="{{ site.baseurl }}/assets/documents/bolinger_CV.pdf#navpanes=0&toolbar=0" type="application/pdf" width="100%" height="600px"> -->
  <!-- <p>Your browser does not support viewing PDFs. Please <a href="{{ site.baseurl }}/assets/documents/bolinger_CV.pdf">download the PDF</a> to view it.</p> -->
<!--   <p>Your browser does not support viewing PDFs. </p> -->
<!-- </object> -->
<!-- <div class="pdf-container"> -->
<object class="pdf-container" data="{{ '/assets/documents/bolinger_CV.pdf#toolbar=0&navpanes=0' | relative_url }}" type="application/pdf" width="95%" height="60%" id="cvPDF">
   <!-- <div class="pdf-fallback" style="padding: 20px; border: 1px solid #ccc; text-align: left;"> -->
  <!-- <span style="display:none;" onerror="document.getElementById('cvPDF').classList.add('failed');"></span> -->
    <!-- <script> -->
      <!-- document.getElementById('cvPDF').className = 'pdf-container.failed'; -->
    <!-- </script> -->
  <div style="padding: 20px; text-align: left;">
    <p>This browser or device does not support inline PDF viewing.</p>
    <!-- <a href="{{ '/assets/documents/bolinger_CV.pdf#toolbar=0&navpanes=0' | relative_url }}" class="btn"> Download and View the PDF </a> -->
  </div>
</object>
<!-- </div> -->

----

<p style="text-align: left; padding-left: 50px; padding-top: 30px;">>> <a href="{{ site.baseurl }}/assets/documents/bolinger_CV.pdf">Download as PDF</a> </p>
